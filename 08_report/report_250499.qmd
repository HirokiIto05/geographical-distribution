...
df_de_country <- read_xlsx(here("01_data", "intermediate", "german", "country_master.xlsx")) |>  
  ...
  mutate(
    region = case_when(
      ...
      country_name == "Serbia (incl. Kosovo) (2006-06-03 to 2008-02-16)" ~ "Serbia",
      country_name == "" ~ "",
      country_name == "" ~ "",
      TRUE ~ "Other"
    )
...
df_de_country |> View()
# df_de_country |> dplyr::filter(str_detect(country_name, "until"))  |> 
df_de_country |> dplyr::filter(is.na(cls_income)) |> distinct(country_name) |> View()
df_de_country |> dplyr::filter(is.na(cls_income)) |> View()
```


```{r}
df_de_country |> dplyr::filter(is.na(iso_code)) |>
  dplyr::distinct(country_name) |> View()

df_country_class |> 
  dplyr::distinct(country_name)  |> View()

list_a <- df_country_class |>
  distinct(country_name) |>
  pull(country_name)

list_b <- df_de_country |> 
  distinct(country_name) |>
  pull(country_name)
df_country_class |> View()
setdiff(list_b, list_a)
```
