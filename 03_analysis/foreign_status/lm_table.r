main <- function() {
  ...
  # classified status ####
  model_high <- lm_five_high(df_master)
  model_low <- lm_five_training(df_master) 
  model_status_total <- lm_five_status(df_master)
  
  table_high_five <- create_lm_kable(model_high, "高技能資格者", 4)
  table_low_five <- create_lm_kable(model_low, "技能実習・特定技能　合計", 4)
  table_status_five <- create_lm_kable(model_status_total, "身分系資格者（特別永住者含む）　合計", 4)
  ...
  write.xlsx(table_high_five, file = "04_analyze/foreign_status/table/high.xlsx")
  write.xlsx(table_low_five, file = "04_analyze/foreign_status/table/training.xlsx")
  write.xlsx(table_status_five, file = "04_analyze/foreign_status/table/status.xlsx")
...
lm_five_high <- function(df_input) {
...
lm_five_training <- function(df_input) {
...
lm_five_status <- function(df_input) {
